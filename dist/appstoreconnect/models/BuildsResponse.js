"use strict";
/* tslint:disable */
/* eslint-disable */
/**
 * App Store Connect API
 * No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)
 *
 * The version of the OpenAPI document: 2.3
 *
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */
Object.defineProperty(exports, "__esModule", { value: true });
exports.BuildsResponseToJSON = exports.BuildsResponseFromJSONTyped = exports.BuildsResponseFromJSON = exports.instanceOfBuildsResponse = void 0;
const runtime_1 = require("../runtime");
const Build_1 = require("./Build");
const BuildsResponseIncludedInner_1 = require("./BuildsResponseIncludedInner");
const PagedDocumentLinks_1 = require("./PagedDocumentLinks");
const PagingInformation_1 = require("./PagingInformation");
/**
 * Check if a given object implements the BuildsResponse interface.
 */
function instanceOfBuildsResponse(value) {
    let isInstance = true;
    isInstance = isInstance && "data" in value;
    isInstance = isInstance && "links" in value;
    return isInstance;
}
exports.instanceOfBuildsResponse = instanceOfBuildsResponse;
function BuildsResponseFromJSON(json) {
    return BuildsResponseFromJSONTyped(json, false);
}
exports.BuildsResponseFromJSON = BuildsResponseFromJSON;
function BuildsResponseFromJSONTyped(json, ignoreDiscriminator) {
    if ((json === undefined) || (json === null)) {
        return json;
    }
    return {
        'data': (json['data'].map(Build_1.BuildFromJSON)),
        'included': !(0, runtime_1.exists)(json, 'included') ? undefined : (json['included'].map(BuildsResponseIncludedInner_1.BuildsResponseIncludedInnerFromJSON)),
        'links': (0, PagedDocumentLinks_1.PagedDocumentLinksFromJSON)(json['links']),
        'meta': !(0, runtime_1.exists)(json, 'meta') ? undefined : (0, PagingInformation_1.PagingInformationFromJSON)(json['meta']),
    };
}
exports.BuildsResponseFromJSONTyped = BuildsResponseFromJSONTyped;
function BuildsResponseToJSON(value) {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
        return null;
    }
    return {
        'data': (value.data.map(Build_1.BuildToJSON)),
        'included': value.included === undefined ? undefined : (value.included.map(BuildsResponseIncludedInner_1.BuildsResponseIncludedInnerToJSON)),
        'links': (0, PagedDocumentLinks_1.PagedDocumentLinksToJSON)(value.links),
        'meta': (0, PagingInformation_1.PagingInformationToJSON)(value.meta),
    };
}
exports.BuildsResponseToJSON = BuildsResponseToJSON;
//# sourceMappingURL=BuildsResponse.js.map