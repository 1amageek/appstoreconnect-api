...
export enum ScmPullRequestsApiApiKeys {
}

export class ScmPullRequestsApi {
    ...
    public setApiKey(key: ScmPullRequestsApiApiKeys, value: string) {
        (this.authentications as any)[ScmPullRequestsApiApiKeys[key]].apiKey = value;
    }
     ...
     */
    public async scmPullRequestsGetInstance (id: string, fieldsScmPullRequests?: Array<'destinationBranchName' | 'destinationRepositoryName' | 'destinationRepositoryOwner' | 'isClosed' | 'isCrossRepository' | 'number' | 'repository' | 'sourceBranchName' | 'sourceRepositoryName' | 'sourceRepositoryOwner' | 'title' | 'webUrl'>, include?: Array<'repository'>, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: ScmPullRequestResponse;  }> {
        const localVarPath = this.basePath + '/v1/scmPullRequests/{id}'
        ...
        if (id === null || id === undefined) {
            throw new Error('Required parameter id was null or undefined when calling scmPullRequestsGetInstance.');
        }
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: ScmPullRequestResponse;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "ScmPullRequestResponse");
