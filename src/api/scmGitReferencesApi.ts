...
export class ScmGitReferencesApi {
    ...
    public setApiKey(key: ScmGitReferencesApiApiKeys, value: string) {
        (this.authentications as any)[ScmGitReferencesApiApiKeys[key]].apiKey = value;
    }
     ...
     */
    public async scmGitReferencesGetInstance (id: string, fieldsScmGitReferences?: Array<'canonicalName' | 'isDeleted' | 'kind' | 'name' | 'repository'>, include?: Array<'repository'>, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: ScmGitReferenceResponse;  }> {
        const localVarPath = this.basePath + '/v1/scmGitReferences/{id}'
        ...
        if (id === null || id === undefined) {
            throw new Error('Required parameter id was null or undefined when calling scmGitReferencesGetInstance.');
        }
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: ScmGitReferenceResponse;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "ScmGitReferenceResponse");
